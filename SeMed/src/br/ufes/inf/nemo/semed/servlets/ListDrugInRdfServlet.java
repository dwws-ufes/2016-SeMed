...
public class ListDrugInRdfServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		if(req.getPathInfo()==null) {
			allDrugsRDFResponse(resp);
		} else {
			trySingleDrugRDFResponse(req,resp);
		}
	}

	private void trySingleDrugRDFResponse(HttpServletRequest req, HttpServletResponse resp) throws IOException {
...
//			String grNS = NSUtils.GR_NS;
	...
	private void allDrugsRDFResponse(HttpServletResponse resp) throws ServletException, IOException {
		resp.setContentType("text/xml");
		List<Drug> drugs = drugDAO.retrieveAll();
		Model model = ModelFactory.createDefaultModel();
		model.setNsPrefix(NSUtils.SEMED_PREFIX, NSUtils.SEMED_NS);
		model.setNsPrefix(NSUtils.GR_PREFIX, NSUtils.GR_NS);
